...
class Posts {
    ...
    function setPostdetails($postdetails) {
        ...
        function htmlpost (){
        return '<pre> Auteur : '.$this->author.'</pre><pre>Titre:'. $this->posttitle.'</pre><pre> Depart:'.$this->departure.'</pre><pre> Date de départ :'.$this->departuredate.'</pre><pre>Heure de départ:'.$this->departuretime.'</pre><pre>Ville d\'arrivée:'.$this->arrivallocation.'</pre><pre> Prix:'.$this->price.'</pre><pre> Plus de détails:'.$this->postdetails.'</pre>';
